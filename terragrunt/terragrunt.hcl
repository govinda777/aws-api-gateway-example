locals {
  ...
  tfstate_dynamodb_table = format("%s-terraform-locks", local.account.aws_account_id)
...
inputs = {
  tag_product       = local.global.product_name
  tag_environment   = local.env.environment_name

  extra_tags = get_terraform_command() == "plan" || (length(regexall(".*eks-worker-nodes.*", path_relative_to_include())) == 1) ? {
